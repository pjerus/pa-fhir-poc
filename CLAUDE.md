...
**Third-fixture facts (2026-08-22, CIGNA-0158):** the pipeline is now dialect-based — one shared mechanism, per-publisher format knowledge in `src/extract/dialects/` (`mac`, `cigna`), sniffed from the PDF's page-1 banner with a filename/id cross-check (fail loud on none/both/mismatch). Spec: `docs/superpowers/specs/2026-08-22-payer-dialect-seam-design.md`. CIGNA-0158 ("Surgical Treatments for Obstructive Sleep Apnea", 32pp) is a single-document commercial policy: no article, zero ICD-10, CPT+HCPCS stance-stratified code tables whose headings ARE the stance statements — so denial reasons are extracted *deterministically* (no LLM) and carry `stance` plus stated `APPLIES_TO` code edges; the policy node holds `DEFINES` directly (Article is MAC-only). Reviewed ground truth: 34 requirements (24 indication / 10 limitation / 0 documentation — faithful; Cigna has no documentation section), 22 covered codes, 4 denial reasons. The PDF is fetch-gated (`tools/fetch-cigna-0158.sh`, gitignored — Cigna policies are copyrighted, unlike public-domain CMS documents); the M1 gate skips loudly when a fixture PDF is absent. **A zero-documentation policy projects NO DTR Questionnaire** — `dtr-std-questionnaire` requires `item` 1..* (validator-verified; `docs/conformance/CIGNA-0158.md`), so projection emits CRD + PlanDefinition only and the CRD card says "coverage criteria apply" with no questionnaire link. Generic defects this fixture exposed, all fixed with regression tests: TOC dot-leader lines anchoring the coding region (whole-line heading matching), and — root cause of every earlier "Ollama flake" — non-streamed completions dying at undici's 300s headers timeout (`llm-client.ts` now streams NDJSON). Validator: CIGNA-0158 PlanDefinition PASS; L33822 re-verified double-PASS post-change.
