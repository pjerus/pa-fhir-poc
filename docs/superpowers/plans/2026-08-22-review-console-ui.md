...
- [ ] **Step 1:** Move `FIXTURES_DIR`, `unionCodes`, `extractAndSnapshot`, `extractArticleAndSnapshot`, `readExtractedSnapshot`, `readArticleSnapshot` (cli.ts, roughly lines 40–176) verbatim into `src/extract/snapshot.ts`, with two changes only: `extractAndSnapshot`/`extractArticleAndSnapshot` take an explicit `llm: LlmClient` second parameter instead of calling `ollamaClient()` internally, and `FIXTURES_DIR` stays the cwd-relative `'fixtures'` string exactly as cli.ts had it — **correction during execution:** the original plan said repo-root-absolute here, which broke `test/cli-project.test.ts`'s temp-cwd isolation contract; cwd-relative is the preserved behavior (the UI server documents "run from repo root", same as the CLI already assumes). Keep the snapshot-shape validation errors word-for-word.
